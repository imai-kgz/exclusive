    ...
    with st.sidebar:
        ...
        for fname in os.listdir(DATA_DIR):
            if fname.endswith('.xlsx') and fname not in st.session_state.files:
                ...
                with open(path, "rb") as f: df = load_data_from_excel(f.read())
...
def main_app():
    ...
    if not st.session_state.files:
        ...
        st.info("Пожалуйста, импортируйте один или несколько Excel файлов в боковой панели.")
        total_sum = 0.0
    else:
                        ...
                        with st.expander(group_name, expanded=bool(search_term)):
                            for idx, row in group_df.iterrows():
                                ...
                                if cols[0].checkbox(row['title'], value=is_checked, key=f"check_{file_name}_{idx}"):
                                    if not is_checked: st.session_state.selected_analyses.setdefault(file_name, []).append(idx)
                                elif is_checked: st.session_state.selected_analyses[file_name].remove(idx)
        ...
        if selected_data:
            ...
            if st.button(f"🧾 Выбрано: {len(selected_data)} поз. на сумму {receipt_sum:,.2f} сом", key="toggle_selected"):
            ...
            if pdf_bytes: bottom_cols[0].download_button("Скачать чек (PDF)", data=pdf_bytes, file_name="check.pdf", mime="application/pdf")
